...
The Mosaic protocol $\cite{Whitepaper}$ issues a USD-pegged stablecoin MEUR that is redeemable at face value against REEF: any owner of MEUR can exchange their stablecoins for equivalent value in the underlying collateral at any time. The REEF paid to the redeemer is taken from the borrowers' collateralized debt positions (“troves”) in ascending order of collateral ratio. In other words, the system uses the redeemed MEUR to repay the debt on the riskiest trove with the currently lowest individual collateral ratio (ICR), and transfers a corresponding amount of REEF from the trove to the redeemer. If the redeemed MEUR is larger than the debt on the riskiest trove, the system proceeds with the second riskiest trove, and so on. \\
...
Troves are collateralized debt positions held by individual borrowers, defined by their amount of debt $d$ (in MEUR) and collateral $c$ (in REEF).
...
If $\gamma_i$ falls below a minimum threshold $\delta$, the trove is subject to liquidation. As long as there are sufficient MEUR tokens in the Stability Pool, the system can use them to repay the liquidated debt. 
