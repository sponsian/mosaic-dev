...
\author{Richard Pardoe \thanks{rick@mosaic.org}\\[1ex]
  Robert Lauko \thanks{robert@mosaic.org} \hspace{2cm}
  Bingen Eguzkitza \thanks{bingen@mosaic.org}
}
...
The Mosaic protocol $\cite{Whitepaper}$ issues a USD-pegged stablecoin MoUSD that is redeemable at face value against REEF: any owner of MoUSD can exchange their stablecoins for equivalent value in the underlying collateral at any time. The REEF paid to the redeemer is taken from the borrowers' collateralized debt positions (“troves”) in ascending order of collateral ratio. In other words, the system uses the redeemed MoUSD to repay the debt on the riskiest trove with the currently lowest individual collateral ratio (ICR), and transfers a corresponding amount of REEF from the trove to the redeemer. If the redeemed MoUSD is larger than the debt on the riskiest trove, the system proceeds with the second riskiest trove, and so on. \\
...
Troves with an insufficient ICR are subject to liquidation. Given that Mosaic's fallback liquidation mechanism $\cite{Whitepaper}$ redistributes the collateral and debt of a liquidated trove between all remaining active troves, it must be ensured that redistributions do not break trove ordering. \\
...
Troves are collateralized debt positions held by individual borrowers, defined by their amount of debt $d$ (in MoUSD) and collateral $c$ (in REEF).
...
If $\gamma_i$ falls below a minimum threshold $\delta$, the trove is subject to liquidation. As long as there are sufficient MoUSD tokens in the Stability Pool, the system can use them to repay the liquidated debt. 
...
In Mosaic, collateral and debt shares from liquidations are proportional to the entire collateral of the recipient troves. The redistribution must cope with the fact that some fraction of a trove's entire collateral is the accumulated reward from prior liquidations, and this fraction varies across troves. \\
...
Given that Mosaic's fallback liquidation mechanism $\cite{Whitepaper}$ redistributes the collateral and debt of a liquidated trove between all remaining active troves, it must be  ensured that redistributions do not break trove ordering.
