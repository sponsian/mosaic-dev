...
The Mosaic protocol uses a Stability Pool as its primary mechanism to liquidate collateralized debt positions (called ``troves'') that fall below the minimum collateralization ratio of 110$\%$ $\cite{Whitepaper}$. Stability deposits (provided in MEUR) absorb and cancel the debt (in MEUR) from defaulted troves: an amount of MEUR in the Stability Pool corresponding to the debt of the liquidated trove is burned from the pool’s balance, offsetting the debt. In return, Stability Pool depositors are rewarded with the acquisition of collateral (in REEF) from liquidated positions at a significant discount. The proportion of a stability depositor’s current deposit to the total MEUR in the pool determines the collateral share it receives from the liquidation. 
...
\section{Derivation}
For a pull-based implementation in Mosaic’s Stability Pool, we must factor out the initial deposit, and track reward terms that are independent from the individual deposit values. We derive formulas for a compounding MEUR deposit and corresponding REEF gain as functions of the initial deposit, and involving a product $P$ and a sum $S$: the former is a function of the MEUR losses, and the latter is a function of $P$, the REEF rewards, and the total deposits.
...
\textbf{Terms}
\begin{terms}
    \item $d_i:$ A given user’s MEUR deposit at liquidation event \textit{i}\\
    \item $Q_i:$ The MEUR debt absorbed by the Stability Pool from liquidation \textit{i}\\
    \item $E_i:$ The REEF sent to the Stability Pool from liquidation \textit{i}\\
    \item $D_i:$ Total MEUR deposits at liquidation \textit{i}
\end{terms}
...
\bigskip
Liquidation assigns an MEUR loss to the deposit proportional to the prior share of total deposits:
...
\bigskip
The MEUR deposit earns an REEF gain at each liquidation.
At each liquidation $i$, the user’s MEUR deposit effectively decreases. Thus we can write the
...
\bigskip
\underline{Upon each liquidation yielding MEUR debt ‘Q’ offset with the Stability Pool and REEF gain ‘E’:}
...
Compute final compounded MEUR deposit $d$: $d = d_t\frac{P}{P_t}$\\
