...
\author{Richard Pardoe \thanks{rick@mosaic.org}}
...
\begin{abstract}
    ...
    Here we derive formulas for a scalable implementation of a compounding, decreasing Stability Pool deposit and its corresponding REEF gain. Thus, Mosaic tracks the deposits and rewards of every depositor with only $O(1)$ complexity for each liquidation.
\end{abstract}

\section{Introduction}
The Mosaic protocol uses a Stability Pool as its primary mechanism to liquidate collateralized debt positions (called ``troves'') that fall below the minimum collateralization ratio of 110$\%$ $\cite{Whitepaper}$. Stability deposits (provided in MoUSD) absorb and cancel the debt (in MoUSD) from defaulted troves: an amount of MoUSD in the Stability Pool corresponding to the debt of the liquidated trove is burned from the pool’s balance, offsetting the debt. In return, Stability Pool depositors are rewarded with the acquisition of collateral (in REEF) from liquidated positions at a significant discount. The proportion of a stability depositor’s current deposit to the total MoUSD in the pool determines the collateral share it receives from the liquidation. 
...
\section{Derivation}
For a pull-based implementation in Mosaic’s Stability Pool, we must factor out the initial deposit, and track reward terms that are independent from the individual deposit values. We derive formulas for a compounding MoUSD deposit and corresponding REEF gain as functions of the initial deposit, and involving a product $P$ and a sum $S$: the former is a function of the MoUSD losses, and the latter is a function of $P$, the REEF rewards, and the total deposits.
...
\textbf{Terms}
\begin{terms}
    \item $d_i:$ A given user’s MoUSD deposit at liquidation event \textit{i}\\
    \item $Q_i:$ The MoUSD debt absorbed by the Stability Pool from liquidation \textit{i}\\
    \item $E_i:$ The REEF sent to the Stability Pool from liquidation \textit{i}\\
    \item $D_i:$ Total MoUSD deposits at liquidation \textit{i}
\end{terms}
...
\bigskip
Liquidation assigns an MoUSD loss to the deposit proportional to the prior share of total deposits:
...
\bigskip
\subsection{Corresponding REEF Gain}

\bigskip
The MoUSD deposit earns an REEF gain at each liquidation.
At each liquidation $i$, the user’s MoUSD deposit effectively decreases. Thus we can write the
user’s cumulative REEF gain $e_{n}$, from a series of \textit{n} liquidations, as:
\begin{equation} 
...
\bigskip
Thus, we have the depositor's cumulative REEF gain $e_n$ as a function of the initial deposit $d_0$, and the rewards and total deposits at each liquidation.\\
Let the summation $\displaystyle\sum^n_{k=1}[...]$ be denoted $S_n$.

\bigskip
\subsection{REEF gain for initial deposit made at $t>0$}
...
\bigskip
We take a snapshot of the sum at liquidation $t$, $S_t$. Then, the REEF gain earned by the deposit at liquidation $n$ is:
\begin{equation} 
...
\bigskip
\underline{Upon each liquidation yielding MoUSD debt ‘Q’ offset with the Stability Pool and REEF gain ‘E’:}

Update $S$: $S = S + \frac{E}{D}P$ (\textit{intuition: a deposit’s marginal REEF gain is equal to the deposit $\times$ REEF per unit staked $\times$ current correction factor)}\\
Update $P$: $P = P(1 - \frac{Q}{D})$\\
Update total deposits: $D = D - Q$

\bigskip
\underline{Withdrawing the deposit and REEF gain:}

Compute final compounded MoUSD deposit $d$: $d = d_t\frac{P}{P_t}$\\
Compute final corresponding cumulative REEF gain $e$: $e = d_t\frac{S - S_t}{P_t}$\\
Send $d$ and $e$ to user\\
Update deposit: deposit[user] = 0\\
Update total deposits: $D = D - d$\\

\bigskip
\section{Practical Implementation in Mosaic}
...
\bigskip
\textbf{Solution}: Complete pool-emptying is handled by tracking a “current epoch” variable. The REEF gain reward sum $S$ for each epoch is stored in a mapping.
...
When users withdraw, check their epoch snapshot against the current epoch: If equal, deposit and REEF gain are computed as normal.
...
\begin{itemize}
    ...
    \item The user’s \textbf{REEF gain} is computed using the $S$ sum that corresponds to the epoch in which their deposit was made.
\end{itemize}
...
\bigskip
If the number of scale changes is 0, compute the deposit and REEF gain as normal.
...
\bigskip
\textbf{REEF Gain:} Since the deposit may span up to one scale change, so too does the reward. In this case, obtain the REEF gain using the $S$ sums from the two consecutive scales that the deposit spans. The reward from the second sum is scaled by 1e-18. The rewards from both scales are added to make the final REEF gain.
...
\bigskip
This allows us to track the REEF reward terms at each scale and epoch.Thus we can correctly compute the REEF rewards for a compounding decreasing stake, taking account of periodic Pool-emptying, and getting around the limitations of Solidity arithmetic.

\bigskip
All implementation logic above is combined in PoolManager.sol, in the functions:

\bigskip
$\_getCompoundedMoUSDDeposit()$
...
\bibitem{Whitepaper} 
R. Lauko, R. Pardoe. 
\textit{Mosaic: Decentralized Borrowing Protocol (Whitepaper), 2020}. 
