...
\begin{abstract}
    ...
    Here we derive formulas for a scalable implementation of a compounding, decreasing Stability Pool deposit and its corresponding ETH gain. Thus, Mosaic tracks the deposits and rewards of every depositor with only $O(1)$ complexity for each liquidation.
\end{abstract}

\section{Introduction}
The Mosaic protocol uses a Stability Pool as its primary mechanism to liquidate collateralized debt positions (called ``troves'') that fall below the minimum collateralization ratio of 110$\%$ $\cite{Whitepaper}$. Stability deposits (provided in MoUSD) absorb and cancel the debt (in MoUSD) from defaulted troves: an amount of MoUSD in the Stability Pool corresponding to the debt of the liquidated trove is burned from the pool’s balance, offsetting the debt. In return, Stability Pool depositors are rewarded with the acquisition of collateral (in ETH) from liquidated positions at a significant discount. The proportion of a stability depositor’s current deposit to the total MoUSD in the pool determines the collateral share it receives from the liquidation. 
...
\section{Derivation}
For a pull-based implementation in Mosaic’s Stability Pool, we must factor out the initial deposit, and track reward terms that are independent from the individual deposit values. We derive formulas for a compounding MoUSD deposit and corresponding ETH gain as functions of the initial deposit, and involving a product $P$ and a sum $S$: the former is a function of the MoUSD losses, and the latter is a function of $P$, the ETH rewards, and the total deposits.
...
\bigskip
The MoUSD deposit earns an ETH gain at each liquidation.
...
\bigskip
\underline{Upon each liquidation yielding MoUSD debt ‘Q’ offset with the Stability Pool and ETH gain ‘E’:}
