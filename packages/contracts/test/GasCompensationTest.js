...
contract('Gas compensation tests', async accounts => {
  ...
  it('_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value', async () => {
    ...
    debt = 10 MEUR
    0.5% of coll = 0.04995 REEF. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 MEUR*/
    ...
    /* REEF:USD price = 200
     ...
     debt = 0 MEUR
     0.5% of coll = 0.000275 REEF. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 MEUR*/
    ...
    // debt = 200 MEUR 
    // 0.5% of coll = 0.004995 REEF. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  MEUR */
  ...
  it('getCompositeDebt(): returns (debt + 50) collateral = $10 in value', async () => {
    ...
    debt = 123.45 MEUR
    0.5% of coll = 0.5 REEF. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 MEUR  */
  ...
  it('getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value', async () => {
    ...
    coll = 100 REEF  
    debt = 2000 MEUR
    -> Expect composite debt = (2000 + 200) = 2200 MEUR  */
    ...
    coll = 37.5 REEF  
    debt = 500 MEUR
    -> Expect composite debt = (500 + 200) = 700 MEUR  */
    ...
    coll = 94758.230582309850 REEF  
    debt = 1 billion MEUR
    -> Expect composite debt = (1000000000 + 200) = 1000000200 MEUR  */
    await priceFeed.setPrice('45323545420000000000000')
    const price_2 = await priceFeed.getPrice()
    ...
    REEF:USD price = 1000000 $/E (1 million)
    coll = 300000000 REEF   (300 million)
    debt = 54321.123456789 MEUR
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 MEUR */
    await priceFeed.setPrice(dec(1, 24))
    const price_3 = await priceFeed.getPrice()
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    ...
    // F opens with 1 REEF, 100 MEUR
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(100, 18), extraParams: { from: flyn } })
    ...
    // C opens with 2.5 REEF, 160 MEUR
    ...
    // E opens with 4405.45 REEF, 32598.35 MEUR
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    // A-E open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(200, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: B_totalDebt.add(C_totalDebt), extraParams: { from: erin } })

    // D, E each provide MEUR to SP
    ...
    const MEURinSP_0 = await stabilityPool.getTotalMEURDeposits()
    ...
    // Check SP MEUR has decreased due to the liquidation 
    const MEURinSP_A = await stabilityPool.getTotalMEURDeposits()
    assert.isTrue(MEURinSP_A.lte(MEURinSP_0))
    ...
    // Check SP MEUR has decreased due to the liquidation of B
    const MEURinSP_B = await stabilityPool.getTotalMEURDeposits()
    assert.isTrue(MEURinSP_B.lt(MEURinSP_A))
    ...
    // Check SP MEUR has decreased due to the liquidation of C
    const MEURinSP_C = await stabilityPool.getTotalMEURDeposits()
    assert.isTrue(MEURinSP_C.lt(MEURinSP_B))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(120, 16)), extraMEURAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extraMEURAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: erin } })

    // D, E each provide 10000 MEUR to SP
    ...
    // Check SP MEUR has decreased due to the liquidation of A
    const MEURinSP_A = await stabilityPool.getTotalMEURDeposits()
    assert.isTrue(MEURinSP_A.lt(MEURinSP_0))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(2000, 18), extraParams: { from: alice,} })
    await openTrove({ ICR: toBN(dec(1875, 15)), extraMEURAmount: dec(8000, 18), extraParams: { from: bob,} })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(600, 18), extraParams: { from: carol} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: dennis} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: erin} })
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: B_totalDebt, extraParams: { from: erin } })
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    // A-F open troves
    await openTrove({ ICR: toBN(dec(118, 16)), extraMEURAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extraMEURAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extraMEURAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extraMEURAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Check MEUR in SP has decreased
    const MEURinSP_1 = await stabilityPool.getTotalMEURDeposits()
    assert.isTrue(MEURinSP_1.lt(MEURinSP_0))
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const MEURinDefaultPool_0 = await defaultPool.getMEURDebt()
    ...
    */
    ...
    // Check MEUR in DefaultPool has decreased
    const MEURinDefaultPool_1 = await defaultPool.getMEURDebt()
    assert.isTrue(MEURinDefaultPool_1.gt(MEURinDefaultPool_0))
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // A-F open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(118, 16)), extraMEURAmount: dec(2000, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(526, 16)), extraMEURAmount: dec(8000, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(488, 16)), extraMEURAmount: dec(600, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(545, 16)), extraMEURAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMEURAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 MEUR
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let debt = 50
    // create 10 troves, constant coll, descending debt 100 to 90 MEUR
    for (const account of _10_accounts) {

      const debtString = debt.toString().concat('000000000000000000')
      await openTrove({ extraMEURAmount: debtString, extraParams: { from: account, value: dec(30, 'ether') } })
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let coll = 50
    // create 20 troves, increasing collateral, constant debt = 100MEUR
    for (const account of _20_accounts) {

      const collString = coll.toString().concat('000000000000000000')
      await openTrove({ extraMEURAmount: dec(100, 18), extraParams: { from: account, value: collString } })
