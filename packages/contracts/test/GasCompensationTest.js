...
contract('Gas compensation tests', async accounts => {
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    // A-E open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: B_totalDebt.add(C_totalDebt), extraParams: { from: erin } })

    // D, E each provide MoUSD to SP
    ...
    const MoUSDinSP_0 = await stabilityPool.getTotalMoUSDDeposits()
    ...
    // Check SP MoUSD has decreased due to the liquidation 
    ...
    // Check SP MoUSD has decreased due to the liquidation of C
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(120, 16)), extraMoUSDAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extraMoUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: erin } })

    // D, E each provide 10000 MoUSD to SP
    ...
    // Check SP MoUSD has decreased due to the liquidation of A
    const MoUSDinSP_A = await stabilityPool.getTotalMoUSDDeposits()
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(2000, 18), extraParams: { from: alice,} })
    await openTrove({ ICR: toBN(dec(1875, 15)), extraMoUSDAmount: dec(8000, 18), extraParams: { from: bob,} })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(600, 18), extraParams: { from: carol} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: dennis} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: erin} })
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: B_totalDebt, extraParams: { from: erin } })
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    // A-F open troves
    await openTrove({ ICR: toBN(dec(118, 16)), extraMoUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extraMoUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extraMoUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extraMoUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Check MoUSD in SP has decreased
    const MoUSDinSP_1 = await stabilityPool.getTotalMoUSDDeposits()
    assert.isTrue(MoUSDinSP_1.lt(MoUSDinSP_0))
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const MoUSDinDefaultPool_0 = await defaultPool.getMoUSDDebt()
    ...
    */
    ...
    // Check MoUSD in DefaultPool has decreased
    const MoUSDinDefaultPool_1 = await defaultPool.getMoUSDDebt()
    assert.isTrue(MoUSDinDefaultPool_1.gt(MoUSDinDefaultPool_0))
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // A-F open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(118, 16)), extraMoUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(526, 16)), extraMoUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(488, 16)), extraMoUSDAmount: dec(600, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(545, 16)), extraMoUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraMoUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 MoUSD
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let debt = 50
    // create 10 troves, constant coll, descending debt 100 to 90 MoUSD
    for (const account of _10_accounts) {

      const debtString = debt.toString().concat('000000000000000000')
      await openTrove({ extraMoUSDAmount: debtString, extraParams: { from: account, value: dec(30, 'ether') } })
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let coll = 50
    // create 20 troves, increasing collateral, constant debt = 100MoUSD
    for (const account of _20_accounts) {

      const collString = coll.toString().concat('000000000000000000')
      await openTrove({ extraMoUSDAmount: dec(100, 18), extraParams: { from: account, value: collString } })
