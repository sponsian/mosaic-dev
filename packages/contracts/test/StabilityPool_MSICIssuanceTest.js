...
contract('StabilityPool - MSIC Rewards', async accounts => {
  ...
  describe("MSIC Rewards", async () => {
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct MSIC gain. No liquidations. No front end.", async () => {
      ...
      // Whale opens Trove with 10k REEF
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(10000, 18)), whale, whale, { from: whale, value: dec(10000, 'ether') })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct MSIC gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(30000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(300, 'ether') })
      ...
      assert.equal(await stabilityPool.getTotalMEURDeposits(), dec(60000, 18))
      ...
      assert.isAtMost(getDifference(await stabilityPool.getTotalMEURDeposits(), dec(30000, 18)), 1000)
    ...
    it('withdrawFromSP(): Depositor withdraws correct MSIC gain after serial pool-emptying liquidations. No front-ends.', async () => {
      ...
      // 4 Defaulters open trove with 200MEUR debt, and 200% ICR
      ...
      // Defaulter 2 liquidated. 10k MEUR offset
      await troveManager.liquidate(defaulter_2, { from: owner });

      // Erin, Flyn each deposit 100 MEUR
      ...
      // Defaulter 3 liquidated. 100 MEUR offset
      await troveManager.liquidate(defaulter_3, { from: owner });

      // Graham, Harriet each deposit 10k MEUR
      ...
      // Defaulter 4 liquidated. 100 MEUR offset
    ...
    A make deposit 10k MEUR
    1 month passes. L1 decreases P: P = 1e-5 P. L1:   9999.9 MEUR, 100 REEF
    B makes deposit 9999.9
    1 month passes. L2 decreases P: P =  1e-5 P. L2:  9999.9 MEUR, 100 REEF
    C makes deposit  9999.9
    1 month passes. L3 decreases P: P = 1e-5 P. L3:  9999.9 MEUR, 100 REEF
    D makes deposit  9999.9
    1 month passes. L4 decreases P: P = 1e-5 P. L4:  9999.9 MEUR, 100 REEF
    E makes deposit  9999.9
    1 month passes. L5 decreases P: P = 1e-5 P. L5:  9999.9 MEUR, 100 REEF
    =========
    F makes deposit 100
    1 month passes. L6 empties the Pool. L6:  10000 MEUR, 100 REEF

    expect A, B, C, D each withdraw ~1 month's worth of MSIC */
    it("withdrawFromSP(): Several deposits of 100 MEUR span one scale factor change. Depositors withdraw correct MSIC gains", async () => {
      // Whale opens Trove with 100 REEF
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(10000, 18)), whale, whale, { from: whale, value: dec(100, 'ether') })
      ...
      for (const defaulter of fiveDefaulters) {
        ...
        await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount('9999900000000000000000'), defaulter, defaulter, { from: defaulter, value: dec(100, 'ether') })
      }

      // Defaulter 6 withdraws to 10k debt (inc. gas comp)
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(10000, 18)), defaulter_6, defaulter_6, { from: defaulter_6, value: dec(100, 'ether') })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct MSIC gain. Front ends and kickback rates", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(50000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(500, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(20000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(200, 'ether') })
      ...
      assert.equal(await stabilityPool.getTotalMEURDeposits(), dec(100000, 18)) // total 100k

      // LIQUIDATION 1
      await troveManager.liquidate(defaulter_1)
      assert.isFalse(await sortedTroves.contains(defaulter_1))

      th.assertIsApproximatelyEqual(await stabilityPool.getTotalMEURDeposits(), dec(50000, 18))  // 50k
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalMEURDeposits(), dec(80000, 18))
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalMEURDeposits(), dec(60000, 18))
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalMEURDeposits(), dec(100000, 18))
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalMEURDeposits(), dec(90000, 18))
      ...
      // Expect deposit C now to be 10125 MEUR
      const C_compoundedMEURDeposit = await stabilityPool.getCompoundedMEURDeposit(C)
      assert.isAtMost(getDifference(C_compoundedMEURDeposit, dec(10125, 18)), 1000)
      ...
      for (depositor of [A, B, C, D, E]) {
        ...
        const compoundedMEURDeposit = await stabilityPool.getCompoundedMEURDeposit(depositor)
        assert.equal(compoundedMEURDeposit, '0')
      }
    ...
    A, B make deposit 5000 MEUR via F1
    1 month passes. L1 depletes P: P = 1e-5*P L1:  9999.9 MEUR, 1 REEF.  scale = 0
    C makes deposit 10000  via F1
    1 month passes. L2 depletes P: P = 1e-5*P L2:  9999.9 MEUR, 1 REEF  scale = 1
    D makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L3:  9999.9 MEUR, 1 REEF scale = 1
    E makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L4:  9999.9 MEUR, 1 REEF scale = 2
    A, B, C, D, E withdraw

    =========
    Expect front end withdraws ~3 month's worth of MSIC */

    it("withdrawFromSP(): Several deposits of 10k MEUR span one scale factor change. Depositors withdraw correct MSIC gains", async () => {
      ...
      for (const defaulter of _4_Defaulters) {
        ...
        await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(99999, 17)), defaulter, defaulter, { from: defaulter, value: dec(100, 'ether') })
      }
