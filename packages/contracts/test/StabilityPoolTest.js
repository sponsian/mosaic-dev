...
contract('StabilityPool', async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    it("provideToSP(): increases totalMoUSDDeposits by correct amount", async () => {
      ...
      // Whale opens Trove with 50 ETH, adds 2000 MoUSD to StabilityPool
    ...
    it("provideToSP(): reverts if cannot receive ETH Gain", async () => {
      ...
      // NonPayable tries to make deposit #2: 100MoUSD (which also attempts to withdraw ETH gain)
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct MoUSD amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
    ...
    it("withdrawFromSP(): it correctly updates the user's MoUSD and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 MoUSD in StabilityPool
      await openTrove({ extraMoUSDAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      // Check A's ETH and MSIC balances have increased correctly
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      //  SETUP: Execute a series of operations to trigger MSIC and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): Applies MoUSDLoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      // --- SETUP ---
      // Whale deposits 185000 MoUSD in StabilityPool
      await openTrove({ extraMoUSDAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
