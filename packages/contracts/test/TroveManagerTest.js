...
contract('TroveManager', async accounts => {
  ...
  const withdrawMoUSD = async (params) => th.withdrawMoUSD(contracts, params)
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    // Alice increases debt to 180 MoUSD, lowering her ICR to 1.11
    const A_MoUSDWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawMoUSD({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1ETH:100MoUSD, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool ETH and MoUSDDebt by correct amounts", async () => {
    ...
    th.assertIsApproximatelyEqual(activePool_MoUSDDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1ETH:100MoUSD, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and MoUSD, 
  ...
  it("liquidate(): increases DefaultPool ETH and MoUSD debt by correct amounts", async () => {
    ...
    assert.equal(defaultPool_MoUSDDebt_Before, '0')
    ...
    const defaultPool_MoUSDDebt_After = (await defaultPool.getMoUSDDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPool_MoUSDDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and MoUSD should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_MoUSDDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1ETH:100MoUSD, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's ether*0.995 and MoUSD should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_ETH()
    const L_MoUSDDebt_AfterCarolLiquidated = await troveManager.L_MoUSDDebt()

    const L_ETH_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_MoUSDDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated, L_ETH_expected_1), 100)
    assert.isAtMost(th.getDifference(L_MoUSDDebt_AfterCarolLiquidated, L_MoUSDDebt_expected_1), 100)

    // Bob now withdraws MoUSD, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawMoUSD({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1ETH:50MoUSD, reducing Bob's ICR below MCR
   ...
   L_MoUSDDebt = (180 / 20) + (890 / 10) = 98 MoUSD */
    ...
    const L_MoUSDDebt_expected_2 = L_MoUSDDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_MoUSDDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
    assert.isAtMost(th.getDifference(L_MoUSDDebt_AfterBobLiquidated, L_MoUSDDebt_expected_2), 100)
  })
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides MoUSD to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraMoUSDAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraMoUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await msicToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides MoUSD to SP
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedMoUSDDeposit(dennis)).toString()
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
    ...
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraMoUSDAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraMoUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides MoUSD to SP
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraMoUSDAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraMoUSDAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraMoUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { msicAmount: A_msicAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { msicAmount: B_msicAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { msicAmount: C_msicAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeMoUSDDebt_0 = await activePool.getMoUSDDebt()
    const defaultMoUSDDebt_0 = await defaultPool.getMoUSDDebt()

    await troveManager.liquidate(alice)
    const activeMoUSDDebt_A = await activePool.getMoUSDDebt()
    const defaultMoUSDDebt_A = await defaultPool.getMoUSDDebt()

    await troveManager.liquidate(bob)
    const activeMoUSDDebt_B = await activePool.getMoUSDDebt()
    const defaultMoUSDDebt_B = await defaultPool.getMoUSDDebt()
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraMoUSDAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
  ...
  it("liquidate(): when SP > 0, triggers MSIC reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalMoUSDDeposits(), dec(100, 18))

    const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1ETH:100MoUSD, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 MoUSD to the SP, but less than C's debt
    ...
    await borrowerOperations.repayMoUSD(dec(1, 18), D, D, {from: D})
    await borrowerOperations.repayMoUSD(dec(1, 18), E, E, {from: E})
    ...
    const pendingMoUSDDebt_C = await troveManager.getPendingMoUSDDebtReward(C)
    const defaultPoolETH = await defaultPool.getETH()
    const defaultPoolMoUSDDebt = await defaultPool.getMoUSDDebt()
    assert.isTrue(pendingETH_C.lte(defaultPoolETH))
    assert.isTrue(pendingMoUSDDebt_C.lte(defaultPoolMoUSDDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingMoUSDDebt_C, defaultPoolMoUSDDebt), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100MoUSD, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraMoUSDAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 MoUSD to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraMoUSDAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 MoUSD
    assert.equal((await stabilityPool.getTotalMoUSDDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalMoUSDDeposits()).toString(), '0')
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 MoUSD in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalMoUSDDeposits()).toString(), totalDeposits)
    ...
    assert.isAtMost(th.getDifference(total_MoUSDinSP, totalDeposits.sub(liquidatedDebt)), 1000)
  ...
  it("liquidateTroves(): when SP > 0, triggers MSIC reward event - increases the sum G", async () => {
    ...
    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraMoUSDAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100MoUSD, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraMoUSDAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalMoUSDDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraMoUSDAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra MoUSD. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided MoUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraMoUSDAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMoUSDAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraMoUSDAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraMoUSDAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_ETHBalance_Before = toBN(await web3.eth.getBalance(dennis))

    const dennis_MoUSDBalance_Before = await msicToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 MoUSD
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    /* check that Dennis' redeemed 20 MoUSD has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) MoUSD debt + 50 for gas compensation. */
    ...
    const dennis_MoUSDBalance_After = (await msicToken.balanceOf(dennis)).toString()
    assert.equal(dennis_MoUSDBalance_After, dennis_MoUSDBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 MoUSD), 20000%
    const { msicAmount: F_msicAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraMoUSDAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 MoUSD
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 MoUSD
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMoUSDAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMoUSDAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMoUSDAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
    ...
    // MoUSD redemption is 55000 US
    const MoUSDRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, MoUSDRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMoUSDAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMoUSDAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMoUSDAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
    ...
    // MoUSD redemption is 55000 MoUSD
    const MoUSDRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, MoUSDRedemption, th._100pct)
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraMoUSDAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraMoUSDAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraMoUSDAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 MoUSD from Carol's Trove
    ...
    // Dennis tries to redeem 20 MoUSD
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 MoUSD from Carol's Trove, Dennis was  able to redeem:
    //  - 9 MoUSD from Carol's
    //  - 8 MoUSD from Bob's
    // for a total of 17 MoUSD.

    // Dennis calculated his hint for redeeming 2 MoUSD from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 MoUSD to fully complete his redemption of 20 MoUSD.
    ...
    const dennis_MoUSDBalance_After = (await msicToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_MoUSDBalance_After, dennis_MoUSDBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveMoUSDAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraMoUSDAmount: amount, extraParams: { from: bob } })
    ...
    const carol_MoUSDBalance_After = (await msicToken.balanceOf(carol)).toString()
    assert.equal(carol_MoUSDBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { msicAmount: B_msicAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraMoUSDAmount: A_debt, extraParams: { from: bob } })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraMoUSDAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraMoUSDAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500MoUSD to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(500, 18), extraParams: { from: alice } })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)

    // Check total MoUSD supply
    ...
    // MoUSD redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedMoUSDRedemption = expectedTotalSupply.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMoUSDRedemption, lessThan5pct), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)  // artificially zero the baseRate
    
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMoUSDRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMoUSDRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMoUSDRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraMoUSDAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraMoUSDAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // MoUSD redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedMoUSDRedemption = expectedTotalSupply.div(toBN(10))

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice()
    const ETHDrawn = attemptedMoUSDRedemption.mul(mv._1e18BN).div(price)
    const slightlyMoreThanFee = (await troveManager.getRedemptionFeeWithDecay(ETHDrawn))
    const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedMoUSDRedemption, slightlyMoreThanFee)
    ...
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedMoUSDRedemption, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedMoUSDRedemption, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedMoUSDRedemption, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedMoUSDRedemption, dec(1, 18))
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraMoUSDAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers MoUSD to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: redemptionAmount, extraParams: { from: alice } })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedMoUSDDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedMoUSDDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedMoUSDDeposit(dennis)).toString()
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedMoUSDDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedMoUSDDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedMoUSDDeposit(dennis)).toString()
  ...
  it("redeemCollateral(): caller can redeem their entire MoUSDToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 MoUSD to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: dec(400, 18), extraParams: { from: alice } })
    ...
    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraMoUSDAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    // Erin attempts to redeem 400 MoUSD
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 MoUSD
  ...
  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's MoUSD token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    // Erin tries to redeem 1000 MoUSD
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's MoUSD token balance")
    }

    // Erin tries to redeem 401 MoUSD
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 MoUSD
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 MoUSD
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed MoUSD (assuming 1 MoUSD has value of $1)", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMoUSDAmount: dec(4990, 18), extraParams: { from: alice } })
    ...
    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraMoUSDAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraMoUSDAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_MoUSD = '120000000000000000000'
    const _373_MoUSD = '373000000000000000000'
    const _950_MoUSD = '950000000000000000000'
    ...
    // Erin redeems 120 MoUSD
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_MoUSD, price, 0))
    ...
    // Flyn redeems 373 MoUSD
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_MoUSD, price, 0))
    ...
    // Graham redeems 950 MoUSD
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_MoUSD, price, 0))
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint MoUSD to Bob
    ...
    // Bob tries to redeem his illegally obtained MoUSD
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    ...
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraMoUSDAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraMoUSDAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getMoUSDDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 MoUSD, from a system that has 100 MoUSD outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check A's balance has decreased by 10 MoUSD
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    // Check B's balance has decreased by 10 MoUSD
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total MoUSD supply
    const activeMoUSD = await activePool.getMoUSDDebt()
    const defaultMoUSD = await defaultPool.getMoUSDDebt()

    const totalMoUSDSupply = activeMoUSD.add(defaultMoUSD)
    th.assertIsApproximatelyEqual(totalMoUSDSupply, totalDebt)
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraMoUSDAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems 360 MoUSD.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    // whale redeems MoUSD.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, GAS_PRICE)
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems MoUSD.  Expect this to fully redeem A, B, C, and partially redeem 15 MoUSD from D.
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraMoUSDAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { msicAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraMoUSDAmount: dec(1, 24), extraParams: { from: alice } })
  ...
  it("getPendingMoUSDDebtReward(): Returns 0 if there is no pending MoUSDDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMoUSDAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraMoUSDAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraMoUSDAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_MoUSDDebt = await troveManager.L_MoUSDDebt()
    assert.equal(current_L_MoUSDDebt, 0)

    const carolSnapshot_L_MoUSDDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_MoUSDDebt, 0)

    const carol_PendingMoUSDDebtReward = await troveManager.getPendingMoUSDDebtReward(carol)
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraMoUSDAmount: B_totalDebt, extraParams: { from: A } })
