...
contract('TroveManager', async accounts => {
  ...
  const getOpenTroveTotalDebt = async (msicAmount) => th.getOpenTroveTotalDebt(contracts, msicAmount)
  ...
  const withdrawMEUR = async (params) => th.withdrawMEUR(contracts, params)
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    // Alice increases debt to 180 MEUR, lowering her ICR to 1.11
    const A_MEURWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawMEUR({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1ETH:100MEUR, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool REEF and MEURDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool REEF and MEUR debt before
    ...
    const activePool_MEURDebt_Before = (await activePool.getMEURDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(activePool_MEURDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1ETH:100MEUR, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and MEUR, 
    leaving Alice’s ether and MEUR debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool REEF and MEUR debt 
    ...
    const activePool_MEURDebt_After = (await activePool.getMEURDebt()).toString()

    assert.equal(activePool_ETH_After, A_collateral)
    assert.equal(activePool_RawEther_After, A_collateral)
    th.assertIsApproximatelyEqual(activePool_MEURDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool REEF and MEUR debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(21, 17)), extraParams: { from: bob } })
    ...
    // check DefaultPool REEF and MEUR debt before
    ...
    const defaultPool_MEURDebt_Before = (await defaultPool.getMEURDebt()).toString()

    assert.equal(defaultPool_ETH_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_MEURDebt_Before, '0')
    ...
    const defaultPool_MEURDebt_After = (await defaultPool.getMEURDebt()).toString()
    ...
    th.assertIsApproximatelyEqual(defaultPool_MEURDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and MEUR should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_MEURDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1ETH:100MEUR, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's ether*0.995 and MEUR should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_ETH()
    const L_MEURDebt_AfterCarolLiquidated = await troveManager.L_MEURDebt()

    const L_ETH_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_MEURDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated, L_ETH_expected_1), 100)
    assert.isAtMost(th.getDifference(L_MEURDebt_AfterCarolLiquidated, L_MEURDebt_expected_1), 100)

    // Bob now withdraws MEUR, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawMEUR({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1ETH:50MEUR, reducing Bob's ICR below MCR
   ...
   L_MEURDebt = (180 / 20) + (890 / 10) = 98 MEUR */
    const L_ETH_AfterBobLiquidated = await troveManager.L_ETH()
    const L_MEURDebt_AfterBobLiquidated = await troveManager.L_MEURDebt()
    ...
    const L_MEURDebt_expected_2 = L_MEURDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_MEURDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
    assert.isAtMost(th.getDifference(L_MEURDebt_AfterBobLiquidated, L_MEURDebt_expected_2), 100)
  })

  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: bob, value: dec(100, 'ether') } })

    // Alice creates a single trove with 0.7 REEF and a debt of 70 MEUR, and provides 10 MEUR to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 MEUR to SP
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides MEUR to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraMEURAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or REEF gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraMEURAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await msicToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides MEUR to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedMEURDeposit(dennis)).toString()
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedMEURDeposit(dennis)).toString()
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or REEF gain", async () => {
    ...
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraMEURAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides MEUR to SP
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and REEF gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraMEURAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraMEURAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    /* Alice's MEUR Loss = (300 / 400) * 200 = 150 MEUR
       Alice's REEF gain = (300 / 400) * 2*0.995 = 1.4925 REEF

       Bob's MEURLoss = (100 / 400) * 200 = 50 MEUR
       Bob's REEF gain = (100 / 400) * 2*0.995 = 0.4975 REEF

     Check Bob' SP deposit has been reduced to 50 MEUR, and his REEF gain has increased to 1.5 REEF. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedMEURDeposit(alice)).toString()
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedMEURDeposit(bob)
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { msicAmount: A_msicAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { msicAmount: B_msicAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { msicAmount: C_msicAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeMEURDebt_0 = await activePool.getMEURDebt()
    const defaultMEURDebt_0 = await defaultPool.getMEURDebt()

    await troveManager.liquidate(alice)
    const activeMEURDebt_A = await activePool.getMEURDebt()
    const defaultMEURDebt_A = await defaultPool.getMEURDebt()

    await troveManager.liquidate(bob)
    const activeMEURDebt_B = await activePool.getMEURDebt()
    const defaultMEURDebt_B = await defaultPool.getMEURDebt()
    ...
    assert.equal((await msicToken.balanceOf(alice)).toString(), A_msicAmount)
    assert.equal((await msicToken.balanceOf(bob)).toString(), B_msicAmount)
    assert.equal((await msicToken.balanceOf(carol)).toString(), C_msicAmount)
  })

  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Defaulter opens with 60 MEUR, 0.6 REEF
    ...
    /* Liquidate defaulter. 30 MEUR and 0.3 REEF is distributed between A, B and C.

    A receives (30 * 2/4) = 15 MEUR, and (0.3*2/4) = 0.15 REEF
    B receives (30 * 1/4) = 7.5 MEUR, and (0.3*1/4) = 0.075 REEF
    C receives (30 * 1/4) = 7.5 MEUR, and (0.3*1/4) = 0.075 REEF
    */
    await troveManager.liquidate(defaulter_1)
  ...
  it("liquidate(): when SP > 0, triggers MSIC reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalMEURDeposits(), dec(100, 18))

    const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1ETH:100MEUR, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // A adds 10 MEUR to the SP, but less than C's debt
    ...
    await borrowerOperations.repayMEUR(dec(1, 18), D, D, {from: D})
    await borrowerOperations.repayMEUR(dec(1, 18), E, E, {from: E})
    ...
    const pendingMEURDebt_C = await troveManager.getPendingMEURDebtReward(C)
    const defaultPoolETH = await defaultPool.getETH()
    const defaultPoolMEURDebt = await defaultPool.getMEURDebt()
    assert.isTrue(pendingETH_C.lte(defaultPoolETH))
    assert.isTrue(pendingMEURDebt_C.lte(defaultPoolMEURDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
    assert.isAtMost(th.getDifference(pendingMEURDebt_C, defaultPoolMEURDebt), 1000)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1ETH:100MEUR, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 MEUR and 0.3 REEF is distributed uniformly between A, B and C. Each receive 10 MEUR, 0.1 REEF
    await troveManager.liquidate(defaulter_1)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraMEURAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await msicToken.balanceOf(dennis)
    const E_balanceBefore = await msicToken.balanceOf(erin)
    const F_balanceBefore = await msicToken.balanceOf(flyn)
    ...
    assert.equal((await msicToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await msicToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await msicToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 MEUR to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraMEURAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 MEUR
    assert.equal((await stabilityPool.getTotalMEURDeposits()).toString(), dec(500, 18))
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and REEF gain", async () => {
    // Whale provides 400 MEUR to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraMEURAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 MEUR in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalMEURDeposits()).toString(), totalDeposits)
    ...
    Total MEUR in Pool: 800 MEUR

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 MEUR
    Total liquidated REEF = 1.1 + 3.1 + 1.1 = 5.3 REEF

    whale msic loss: 650 * (400/800) = 325 msic
    alice msic loss:  650 *(100/800) = 81.25 msic
    bob msic loss: 650 * (300/800) = 243.75 msic

    whale remaining deposit: (400 - 325) = 75 msic
    alice remaining deposit: (100 - 81.25) = 18.75 msic
    bob remaining deposit: (300 - 243.75) = 56.25 msic
    ...
    Total remaining deposits: 150 MEUR
    Total REEF gain: 4.975 REEF */

    // Check remaining MEUR Deposits and REEF gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedMEURDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedMEURDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedMEURDeposit(bob)
    ...
    const total_MEURinSP = (await stabilityPool.getTotalMEURDeposits()).toString()
    const total_ETHinSP = (await stabilityPool.getETH()).toString()

    assert.isAtMost(th.getDifference(total_MEURinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): when SP > 0, triggers MSIC reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraMEURAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1ETH:100MEUR, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraMEURAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalMEURDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await msicToken.transfer(carol, dec(100, 18), { from: whale })
  ...
  it("batchLiquidateTroves: when SP > 0, triggers MSIC reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraMEURAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra MEUR. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided MEUR with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraMEURAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMEURAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraMEURAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraMEURAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_ETHBalance_Before = toBN(await web3.eth.getBalance(dennis))

    const dennis_MEURBalance_Before = await msicToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 MEUR
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    /* check that Dennis' redeemed 20 MEUR has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) MEUR debt + 50 for gas compensation. */
    ...
    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount MEUR to REEF, at REEF:USD price 200
    ...
    const dennis_MEURBalance_After = (await msicToken.balanceOf(dennis)).toString()
    assert.equal(dennis_MEURBalance_After, dennis_MEURBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 MEUR), 20000%
    const { msicAmount: F_msicAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraMEURAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 MEUR
    const flynBalance = await msicToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(flynBalance, F_msicAmount.sub(redemptionAmount))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 MEUR
    const flynBalance = (await msicToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(flynBalance, F_msicAmount.sub(redemptionAmount))
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })

    // A and C send all their tokens to B
    await msicToken.transfer(B, await msicToken.balanceOf(A), {from: A})
    await msicToken.transfer(B, await msicToken.balanceOf(C), {from: C})
    ...
    // MEUR redemption is 55000 US
    const MEURRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, MEURRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveMEURAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraMEURAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraMEURAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraMEURAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 MEUR from Carol's Trove
    ...
    // Dennis tries to redeem 20 MEUR
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 MEUR from Carol's Trove, Dennis was  able to redeem:
    //  - 9 MEUR from Carol's
    //  - 8 MEUR from Bob's
    // for a total of 17 MEUR.

    // Dennis calculated his hint for redeeming 2 MEUR from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 MEUR to fully complete his redemption of 20 MEUR.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted MEUR converted to REEF, at REEF:USD price 200
    ...
    const dennis_MEURBalance_After = (await msicToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_MEURBalance_After, dennis_MEURBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveMEURAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraMEURAmount: amount, extraParams: { from: bob } })

    await msicToken.transfer(carol, amount, { from: bob })
    ...
    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 MEUR to REEF at REEF:USD price of 100
    ...
    const carol_MEURBalance_After = (await msicToken.balanceOf(carol)).toString()
    assert.equal(carol_MEURBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { msicAmount: B_msicAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraMEURAmount: A_debt, extraParams: { from: bob } })

    await msicToken.transfer(carol, B_msicAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraMEURAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraMEURAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500MEUR to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(500, 18), extraParams: { from: alice } })
    await msicToken.transfer(erin, dec(500, 18), { from: alice })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)

    // Check total MEUR supply
    ...
    // MEUR redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedMEURRedemption = expectedTotalSupply.div(toBN(10))

    // Max fee is <5%
    const lessThan5pct = '49999999999999999'
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMEURRedemption, lessThan5pct), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)  // artificially zero the baseRate
    
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMEURRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMEURRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedMEURRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraMEURAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraMEURAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // MEUR redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedMEURRedemption = expectedTotalSupply.div(toBN(10))
    ...
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedMEURRedemption, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedMEURRedemption, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedMEURRedemption, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedMEURRedemption, dec(1, 18))
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or REEF gain of redeemed-from troves", async () => {
    ...
    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraMEURAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMEURAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers MEUR to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: redemptionAmount, extraParams: { from: alice } })
    await msicToken.transfer(erin, redemptionAmount, { from: alice })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedMEURDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedMEURDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedMEURDeposit(dennis)).toString()

    const bob_ETHGain_before = (await stabilityPool.getDepositorETHGain(bob)).toString()
    const carol_ETHGain_before = (await stabilityPool.getDepositorETHGain(carol)).toString()
    const dennis_ETHGain_before = (await stabilityPool.getDepositorETHGain(dennis)).toString()

    // Check the remaining MEUR and REEF in Stability Pool after liquidation is non-zero
    const MEURinSP = await stabilityPool.getTotalMEURDeposits()
    const ETHinSP = await stabilityPool.getETH()
    assert.isTrue(MEURinSP.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedMEURDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedMEURDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedMEURDeposit(dennis)).toString()
  ...
  it("redeemCollateral(): caller can redeem their entire MEURToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 MEUR to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: dec(400, 18), extraParams: { from: alice } })
    await msicToken.transfer(erin, dec(400, 18), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await msicToken.balanceOf(erin)
    assert.equal(erin_balance_before, dec(400, 18))

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraMEURAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    // Erin attempts to redeem 400 MEUR
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 MEUR
    const activePool_debt_after = await activePool.getMEURDebt()
    ...
    const erin_balance_after = (await msicToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's MEUR token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    // Erin tries to redeem 1000 MEUR
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's MEUR token balance")
    }

    // Erin tries to redeem 401 MEUR
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 MEUR
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 MEUR
  ...
  it("redeemCollateral(): value of issued REEF == face value of redeemed MEUR (assuming 1 MEUR has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 1000 MEUR each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraMEURAmount: dec(4990, 18), extraParams: { from: alice } })
    await msicToken.transfer(erin, dec(1000, 18), { from: alice })
    await msicToken.transfer(flyn, dec(1000, 18), { from: alice })
    await msicToken.transfer(graham, dec(1000, 18), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraMEURAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraMEURAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraMEURAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_MEUR = '120000000000000000000'
    const _373_MEUR = '373000000000000000000'
    const _950_MEUR = '950000000000000000000'
    ...
    // Erin redeems 120 MEUR
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_MEUR, price, 0))
    ...
    /* 120 MEUR redeemed.  Expect $120 worth of REEF removed. At REEF:USD price of $200, 
    ...
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_MEUR, price, 0))
    ...
    /* 373 MEUR redeemed.  Expect $373 worth of REEF removed. At REEF:USD price of $200, 
    ...
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_MEUR, price, 0))
    ...
    /* 950 MEUR redeemed.  Expect $950 worth of REEF removed. At REEF:USD price of $200, 
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint MEUR to Bob
    await msicToken.unprotectedMint(bob, dec(100, 18))

    assert.equal((await msicToken.balanceOf(bob)), dec(100, 18))
    ...
    // Bob tries to redeem his illegally obtained MEUR
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint MEUR to Bob
    await msicToken.unprotectedMint(bob, '101000000000000000000')

    assert.equal((await msicToken.balanceOf(bob)), '101000000000000000000')

    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraMEURAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraMEURAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getMEURDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 MEUR, from a system that has 100 MEUR outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await msicToken.balanceOf(A)

    await th.redeemCollateral(A, contracts, dec(10, 18), GAS_PRICE)

    // Check A's balance has decreased by 10 MEUR
    assert.equal(await msicToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    // Check B's balance has decreased by 10 MEUR
    assert.equal(await msicToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 MEUR
    assert.equal(A_balanceBefore.sub(await msicToken.balanceOf(A)), dec(10, 18))
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to MSIC staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MSIC
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await msicToken.approve(msicStaking.address, dec(1, 18), { from: multisig })
    await msicStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check MSIC Staking contract balance before is zero
    const msicStakingBalance_Before = await web3.eth.getBalance(msicStaking.address)
    assert.equal(msicStakingBalance_Before, '0')
    ...
    // Check MSIC Staking contract balance after is non-zero
    const msicStakingBalance_After = toBN(await web3.eth.getBalance(msicStaking.address))
  ...
  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to MSIC staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 MSIC
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await msicToken.approve(msicStaking.address, dec(1, 18), { from: multisig })
    await msicStaking.stake(dec(1, 18), { from: multisig })
    ...
    const msicStakingBalance_Before = toBN(await web3.eth.getBalance(msicStaking.address))
    ...
    // check MSIC Staking balance has increased
  ...
  it("redeemCollateral(): a redemption sends the REEF remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total MEUR supply
    const activeMEUR = await activePool.getMEURDebt()
    const defaultMEUR = await defaultPool.getMEURDebt()

    const totalMEURSupply = activeMEUR.add(defaultMEUR)
    th.assertIsApproximatelyEqual(totalMEURSupply, totalDebt)
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraMEURAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems 360 MEUR.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    // whale redeems MEUR.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, GAS_PRICE)
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems MEUR.  Expect this to fully redeem A, B, C, and partially redeem 15 MEUR from D.
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's REEF surplus (collateral - REEF drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraMEURAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { msicAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraMEURAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(msicAmount, price, 0)
  ...
  it("getPendingMEURDebtReward(): Returns 0 if there is no pending MEURDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraMEURAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraMEURAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraMEURAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_MEURDebt = await troveManager.L_MEURDebt()
    assert.equal(current_L_MEURDebt, 0)

    const carolSnapshot_L_MEURDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_MEURDebt, 0)

    const carol_PendingMEURDebtReward = await troveManager.getPendingMEURDebtReward(carol)
    assert.equal(carol_PendingMEURDebtReward, 0)
  })
  ...
  it("computeICR(): Returns 2^256-1 for REEF:USD = 100, coll = 1 REEF, debt = 100 MEUR", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for REEF:USD = 100, coll = 200 REEF, debt = 30 MEUR", async () => {
    const price = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for REEF:USD = 250, coll = 1350 REEF, debt = 127 MEUR", async () => {
  ...
  it("computeICR(): returns correct ICR for REEF:USD = 100, coll = 1 REEF, debt = 54321 MEUR", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraMEURAmount: B_totalDebt, extraParams: { from: A } })

    // to be able to repay:
    await msicToken.transfer(B, B_totalDebt, { from: A })
