...
contract('DefaultPool', async accounts => {
  ...
  it('sendETHToActivePool(): fails if receiver cannot receive REEF', async () => {
    ...
    //await th.assertRevert(defaultPool.sendETHToActivePool(amount, { from: owner }), 'DefaultPool: sending REEF failed')
    const sendETHData = th.getTransactionData('sendETHToActivePool(uint256)', [web3.utils.toHex(amount)])
    await th.assertRevert(mockTroveManager.forward(defaultPool.address, sendETHData, { from: owner }), 'DefaultPool: sending REEF failed')
  })
})
