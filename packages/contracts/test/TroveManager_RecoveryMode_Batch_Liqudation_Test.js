...
contract('TroveManager - in Recovery Mode - back to normal mode in 1 tx', async accounts => {
  ...
  context('Batch liquidations', () => {
    const setup = async () => {
      ...
      await openTrove({ ICR: toBN(dec(340, 16)), extraMoUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
    ...
    it('Stability Pool profit matches', async () => {
      ...
      const spMousdBefore = await stabilityPool.getTotalMoUSDDeposits()
      ...
      const spMousdAfter = await stabilityPool.getTotalMoUSDDeposits()
    ...
    it('A trove over TCR is not liquidated', async () => {
      ...
      await openTrove({ ICR: toBN(dec(310, 16)), extraMoUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
      await stabilityPool.provideToSP(totalLiquidatedDebt, ZERO_ADDRESS, { from: whale })
  ...
  context('Sequential liquidations', () => {
    const setup = async () => {
      ...
      await openTrove({ ICR: toBN(dec(300, 16)), extraMoUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
      await stabilityPool.provideToSP(totalLiquidatedDebt, ZERO_ADDRESS, { from: whale })
