...
MEUR_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for MEUR. The ether price $P_t^e$ follows 
...
> (collateral in ether, debt in MEUR, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of MEUR.

$N_t^o$ is rounded to an integer.

---

The amount of MEUR tokens generated by a new trove is
...
#sensitivity to MEUR price & issuance fee
alpha = 0.3
...
- $D_{t}^{s}$ is the total amount of MEUR deposited in the Stability Pool (see below)
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalMEURDeposits()
    trove = last_trove
...
def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MEUR, price_MSIC_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalMEURDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalMEURDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_MEUR
    ...
    return_stability = calculate_stability_return(contracts, price_MEUR, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_MEUR, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalMEURDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_MEUR * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MEUR, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_MEUR)*n_troves
...
def get_msic_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    ...
    if debt > msicBalance:
        ...
        # try with active accounts, which are more likely to hold MEUR
        ...
        if pending > 0:
            print(f"\n ***Error: not enough MEUR to repay! {debt / 1e18} MEUR for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                if pending == 0:
                    contracts.borrowerOperations.repayMEUR(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawMEUR(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_MEUR_adjust = issuance_MEUR_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_MEUR_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + MEUR_GAS_COMPENSATION
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MEUR, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_MEUR_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_MEUR <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_MEUR - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_MEUR_open = issuance_MEUR_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_MEUR_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    ...
    if stability_pool > stability_pool_previous:
        ...
        current_deposit = contracts.stabilityPool.getCompoundedMEURDeposit(accounts[0])
...
"""MEUR Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of MEUR tokens defined above, the price of MEUR at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of MEUR.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_MEUR, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_MEUR_current= price_MEUR * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_MEUR_current

""" **Stabilizers**

There are two stabilizers to attenuate MEUR price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the MEUR price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of MEUR in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of MEUR price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    ...
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedMEURamount] = contracts.hintHelpers.getRedemptionHints(msic_balance, price_ether_current, 70)
    if truncatedMEURamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"MEUR Token:    {contracts.msicToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"MEUR bal: {msic_balance / 1e18}")
        print(f"truncated: {truncatedMEURamount / 1e18}")
        ...
        print(f"amount: {truncatedMEURamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MEUR, index):

    stability_pool = contracts.stabilityPool.getTotalMEURDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_MEUR_stabilizer = 0
    ...
    #Calculating Price
    price_MEUR_current = calculate_price(price_MEUR, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_MEUR_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_MEUR)**delta
        ...
        issuance_MEUR_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_MEUR_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_MEUR_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_MEUR)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_MEUR_current = 1 - rate_redemption
        else:
    ...
    return [price_MEUR_current, redemption_pool, redemption_fee, issuance_MEUR_stabilizer]
