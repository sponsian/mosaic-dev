...
MoUSD_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for MoUSD. The ether price $P_t^e$ follows 
...
"""# MSIC price
In the first month, the price of MSIC follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_MSIC) represents MSIC price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_MSIC, so that the expected MSIC price increases from price_MSIC_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_MSIC_initial$ \cdot (1+$ drift_MSIC$)^{720}$

The MSIC price from the second month on is endogenously determined.
"""

#MSIC price & airdrop
price_MSIC_initial = 0.4
price_MSIC = [price_MSIC_initial]
sd_MSIC=0.005
drift_MSIC = 0.0035
supply_MSIC=[0]
MSIC_total_supply=100000000

"""**MSIC Endogenous Price**

The staked MSIC pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{MSIC Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ MSIC_total_supply

Therefore, the MSIC price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{MSIC\_total\_supply}E_t$$

Interpretation: The denominator implies that with more MSIC tokens issued, MSIC price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop MSIC gain.
...
> (collateral in ether, debt in MoUSD, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of MoUSD.

$N_t^o$ is rounded to an integer.

---

The amount of MoUSD tokens generated by a new trove is
...
#sensitivity to MoUSD price & issuance fee
alpha = 0.3
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of MSIC token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of MoUSD deposited in the Stability Pool (see below)
...
print(" - REEF period 1 -")
print(f"Min REEF price: {min(price_ether[1:period1])}")
print(f"Max REEF price: {max(price_ether[1:period1])}")
for i in range(period1, period2):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether2))
print(" - REEF period 2 -")
print(f"Min REEF price: {min(price_ether[period1:period2])}")
print(f"Max REEF price: {max(price_ether[period1:period2])}")
for i in range(period2, period3):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether3))
print(" - REEF period 3 -")
print(f"Min REEF price: {min(price_ether[period2:period3])}")
print(f"Max REEF price: {max(price_ether[period2:period3])}")
for i in range(period3, period4):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether4))
print(" - REEF period 4 -")
print(f"Min REEF price: {min(price_ether[period3:period4])}")
print(f"Max REEF price: {max(price_ether[period3:period4])}")
...
"""MSIC Price - First Month"""

#MSIC price
for i in range(1, month):
    random.seed(2+13*i)
    shock_MSIC = random.normalvariate(0,sd_MSIC)  
    price_MSIC.append(price_MSIC[i-1]*(1+shock_MSIC)*(1+drift_MSIC))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalMoUSDDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_MSIC_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MoUSD, price_MSIC_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalMoUSDDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalMoUSDDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_MoUSD
    airdrop_gain = price_MSIC_current * quantity_MSIC_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_MoUSD, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_MoUSD, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalMoUSDDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_MoUSD * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MoUSD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_MoUSD)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_msic_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.msicToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.msicToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_msic_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    msicBalance = contracts.msicToken.balanceOf(account)
    if debt > msicBalance:
        pending = debt - msicBalance
        ...
        # try with active accounts, which are more likely to hold MoUSD
        ...
        if pending > 0:
            print(f"\n ***Error: not enough MoUSD to repay! {debt / 1e18} MoUSD for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_msic_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayMoUSD(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawMoUSD(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_MoUSD_adjust = issuance_MoUSD_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_MoUSD_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + MoUSD_GAS_COMPENSATION
    msic = get_msic_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, msic, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MoUSD, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_MoUSD_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_MoUSD <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_MoUSD - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_MoUSD_open = issuance_MoUSD_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_MoUSD_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.msicToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalMoUSDDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.msicToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedMoUSDDeposit(accounts[0])
...
"""MoUSD Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of MoUSD tokens defined above, the price of MoUSD at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of MoUSD.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_MoUSD, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_MoUSD_current= price_MoUSD * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_MoUSD_current

""" **Stabilizers**

There are two stabilizers to attenuate MoUSD price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the MoUSD price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of MoUSD in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of MoUSD price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    msic_balance = contracts.msicToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedMoUSDamount] = contracts.hintHelpers.getRedemptionHints(msic_balance, price_ether_current, 70)
    if truncatedMoUSDamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"MoUSD Token:    {contracts.msicToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"MoUSD bal: {msic_balance / 1e18}")
        print(f"truncated: {truncatedMoUSDamount / 1e18}")
        ...
        print(f"amount: {truncatedMoUSDamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_MoUSD, index):

    stability_pool = contracts.stabilityPool.getTotalMoUSDDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_MoUSD_stabilizer = 0
    ...
    #Calculating Price
    price_MoUSD_current = calculate_price(price_MoUSD, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_MoUSD_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_MoUSD)**delta
        ...
        issuance_MoUSD_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_MoUSD_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_MoUSD_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_MoUSD)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_MoUSD_current = 1 - rate_redemption
        else:
    ...
    return [price_MoUSD_current, redemption_pool, redemption_fee, issuance_MoUSD_stabilizer]

"""# MSIC Market"""

def MSIC_market(index, data):
    #quantity_MSIC = (MSIC_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_MSIC_current = price_MSIC[index-1]
        ...
        price_MSIC_current = discount * PE_ratio * annualized_earning / MSIC_total_supply

    #MC_MSIC_current = price_MSIC_current * quantity_MSIC

    return [price_MSIC_current, annualized_earning]
