...
def setAddresses(contracts):
    ...
    # MSIC
    contracts.msicStaking.setAddresses(
        contracts.msicToken.address,
        contracts.msicToken.address,
        contracts.troveManager.address,
        contracts.borrowerOperations.address,
...
def contracts():
    ...
    # MSIC
    contracts.msicStaking = MSICStaking.deploy({ 'from': accounts[0] })
    ...
    contracts.msicToken = MSICToken.deploy(
        contracts.communityIssuance.address,
        contracts.msicStaking.address,
...
def print_expectations():
    ...
    print("Expected MSIC price at the end of first month: $", price_MSIC_initial * (1 + drift_MSIC)**720)
...
* trove liquidation
* return of the previous period's stability pool determined (liquidation gain & airdropped MSIC gain)
* trove closure
* trove adjustment
* open troves
* issuance fee
...
def test_run_simulation(add_accounts, contracts, print_expectations):
    ...
    with open('tests/simulation.csv', 'w', newline='') as csvfile:
        ...
        for index in range(1, n_sim):
            ...
            #MSIC Market
            result_MSIC = MSIC_market(index, data)
            price_MSIC_current = result_MSIC[0]
