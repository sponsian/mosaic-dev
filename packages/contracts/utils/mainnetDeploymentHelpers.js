...
class MainnetDeploymentHelper {
  ...
  async deployMosaicCoreMainnet(tellorMasterAddr, deploymentState) {
    ...
    const msicTokenParams = [
      troveManager.address,
      stabilityPool.address,
      borrowerOperations.address
    ]
    const msicToken = await this.loadOrDeploy(
      msicTokenFactory,
      'msicToken',
      deploymentState,
      msicTokenParams
    ...
    if (!this.configParams.ETHERSCAN_BASE_URL) {
      ...
      await this.verifyContract('msicToken', deploymentState, msicTokenParams)
    }
  ...
  async deployMSICContractsMainnet(bountyAddress, lpRewardsAddress, multisigAddress, deploymentState) {
    const msicStakingFactory = await this.getFactory("MSICStaking")
    const lockupContractFactory_Factory = await this.getFactory("LockupContractFactory")
    const communityIssuanceFactory = await this.getFactory("CommunityIssuance")
    const msicTokenFactory = await this.getFactory("MSICToken")

    const msicStaking = await this.loadOrDeploy(msicStakingFactory, 'msicStaking', deploymentState)
    ...
    if (!this.configParams.ETHERSCAN_BASE_URL) {
      ...
      await this.verifyContract('msicStaking', deploymentState)
  ...
  async deployMultiTroveGetterMainnet(mosaicCore, deploymentState) {
    const multiTroveGetterFactory = await this.getFactory("MultiTroveGetter")
    const multiTroveGetterParams = [
      mosaicCore.troveManager.address,
      mosaicCore.sortedTroves.address
    ]
  ...
  async connectCoreContractsMainnet(contracts, MSICContracts, chainlinkProxyAddress) {
  ...
  async connectMSICContractsMainnet(MSICContracts) {
    const gasPrice = this.configParams.GAS_PRICE
    // Set MSICToken address in LCF
    await this.isOwnershipRenounced(MSICContracts.msicStaking) ||
      await this.sendAndWaitForTransaction(MSICContracts.lockupContractFactory.setMSICTokenAddress(MSICContracts.msicToken.address, {gasPrice}))
  }

  async connectMSICContractsToCoreMainnet(MSICContracts, coreContracts) {
    const gasPrice = this.configParams.GAS_PRICE
    await this.isOwnershipRenounced(MSICContracts.msicStaking) ||
      await this.sendAndWaitForTransaction(MSICContracts.msicStaking.setAddresses(
        MSICContracts.msicToken.address,
    ...
    await this.isOwnershipRenounced(MSICContracts.communityIssuance) ||
      await this.sendAndWaitForTransaction(MSICContracts.communityIssuance.setAddresses(
        MSICContracts.msicToken.address,
