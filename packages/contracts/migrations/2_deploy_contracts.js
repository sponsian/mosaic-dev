...
module.exports = function(deployer) {
  ...
  deployer.then(async () => {
    ...
    const msicToken = await MoUSDToken.deployed()
    const functionCaller = await FunctionCaller.deployed()

    const mosaicContracts = {
      borrowerOperations,
      priceFeed,
      msicToken,
      sortedTroves,
    ...
    const mosaicAddresses = getAddresses(mosaicContracts)
    console.log('deploy_contracts.js - Deployed contract addresses: \n')
    console.log(mosaicAddresses)
    console.log('\n')

    // Connect contracts to each other
    await connectContracts(mosaicContracts, mosaicAddresses)
  })
}
