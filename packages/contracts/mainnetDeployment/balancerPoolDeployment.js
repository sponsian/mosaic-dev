...
const MoUSD = '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0';
const CHAINLINK_ETHUSD_PROXY = '0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419';

const tokens = [MoUSD, WETH];
const weights = [toBigNum(dec(4, 17)), toBigNum(dec(6, 17))];

const NAME = 'WETH/MoUSD Pool';
const SYMBOL = '60WETH-40MoUSD';
...
async function main() {
  ...
  */
  ...
  const msic_balance = INITIAL_FUNDING.mul(weights[0]).div(toBigNum(dec(1, 18)));
  const initialBalances = [msic_balance, weth_balance];
  th.logBN('Initial MoUSD', msic_balance);
  ...
  // Approve MoUSD
  const msic = new ethers.Contract(
    MoUSD,
    ERC20.abi,
    deployerWallet
  );
  const txApproveMousd = await msic.approve(VAULT, msic_balance);
  const approveMousdReceipt = await txApproveMousd.wait();
  console.log('Approve MoUSD gas: ', approveMousdReceipt.gasUsed.toString());
  ...
  console.log('Total gas: ', receipt1.gasUsed.add(depositWethReceipt.gasUsed).add(approveWethReceipt.gasUsed).add(approveMousdReceipt.gasUsed).add(receipt2.gasUsed).toString());
}
