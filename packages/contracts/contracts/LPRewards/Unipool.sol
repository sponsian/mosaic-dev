...
// Incremental changes (commit by commit) from the original to this version: https://github.com/mosaic/dev/pull/271
...
/*
 * On deployment a new Uniswap pool will be created for the pair MoUSD/REEF and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of MSIC token,
 ...
 * in order to avoid getting MSIC tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either MSIC token contract is deployed, and therefore MSIC tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
        ...
        onlyOwner
    {
        ...
        _notifyRewardAmount(msicToken.getLpRewardsEntitlement(), _duration);
    ...
    function lastTimeRewardApplicable() public view override returns (uint256) {
        return MosaicMath._min(block.timestamp, periodFinish);
    }
    ...
    function claimReward() public override {
        ...
        msicToken.transfer(msg.sender, reward);
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == msicToken.balanceOf(address(this)));
