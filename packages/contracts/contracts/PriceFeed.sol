...
/*
* PriceFeed for mainnet deployment, to be connected to Chainlink's live REEF:EUR aggregator reference
...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    // Core Mosaic contracts
    address borrowerOperationsAddress;
    address troveManagerAddress;

    uint constant public REEFEUR_TELLOR_REQ_ID = 1;
    ...
    */
    ...
    // The last good price seen from an oracle by Mosaic
    ...
    /*
    * fetchPrice():
    * Returns the latest price obtained from the Oracle. Called by Mosaic functions that require a current price.
    *
    * Also callable by anyone externally.
    *
    * Non-view function - it stores the last good price seen by Mosaic.
    *
    * Uses a main oracle (Chainlink) and a fallback oracle (Tellor) in case Chainlink fails. If both fail, 
    * it uses the last good price seen by Mosaic.
    *
    */
    ...
    */
    ...
    function _chainlinkPriceChangeAboveMax(ChainlinkResponse memory _currentResponse, ChainlinkResponse memory _prevResponse) internal pure returns (bool) {
        ...
        uint minPrice = MosaicMath._min(currentScaledPrice, prevScaledPrice);
        uint maxPrice = MosaicMath._max(currentScaledPrice, prevScaledPrice);
    ...
    function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits) internal pure returns (uint) {
        /*
        * Convert the price returned by the Chainlink oracle to an 18-digit decimal for use by Mosaic.
        * At date of Mosaic launch, Chainlink uses an 8-digit price, but we also handle the possibility of
        * future changes.
        *
        */
        uint price;
    ...
    function _getCurrentTellorResponse() internal view returns (TellorResponse memory tellorResponse) {
        try tellorCaller.getTellorCurrentValue(REEFEUR_TELLOR_REQ_ID) returns
        (
            bool ifRetrieve,
            uint256 value,
