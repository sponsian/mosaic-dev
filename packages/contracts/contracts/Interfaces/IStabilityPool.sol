...
interface IStabilityPool {
    ...
    /*
     * Called only once on init, to set addresses of other Mosaic contracts
    ...
    /*
     * Returns the total amount of REEF held by the pool, accounted in an internal variable instead of `balance`,
     * to exclude edge cases like REEF received from a self-destruct.
     */
    function getETH() external view returns (uint);

    /*
     * Returns MoUSD held in the pool. Changes when users deposit/withdraw, and when Trove debt is offset.
     */
    function getTotalMoUSDDeposits() external view returns (uint);
     ...
     */
    function getDepositorMSICGain(address _depositor) external view returns (uint);

    /*
     * Return the MSIC gain earned by the front end.
     */
    function getFrontEndMSICGain(address _frontEnd) external view returns (uint);

    /*
     * Return the user's compounded deposit.
     */
    function getCompoundedMoUSDDeposit(address _depositor) external view returns (uint);
    ...
    /*
     * Fallback function
     * Only callable by Active Pool, it just accounts for REEF received
