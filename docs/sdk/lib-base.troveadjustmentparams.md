...
[Home](./index.md) &gt; [@mosaic/lib-base](./lib-base.md) &gt; [TroveAdjustmentParams](./lib-base.troveadjustmentparams.md)

## TroveAdjustmentParams type

Parameters of an [adjustTrove()](./lib-base.transactablemosaic.adjusttrove.md) transaction.
...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowMEUR` and `repayMEUR` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowMEUR? </td> <td> T </td> <td> <i>(Optional)</i> The amount of MEUR that's borrowed. </td> </tr>
